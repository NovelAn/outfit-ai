...
衣橱单品详情浮层支持“编辑信息”和“删除单品”。编辑保存调用 `PATCH` 并立即更新当前卡片；编辑页不要求品牌字段，厚薄度使用 `轻薄`、`适中`、`厚实` 受控标签保存，不新增数据库列。弹窗层级高于固定底部导航，长表单可在弹窗内滚动。删除必须在浮层内二次确认，后端会同时删除数据库记录、原图和 `.nobg` 图片；图片替换、手工抠图和重新调用 AI 不在当前版本范围内。
